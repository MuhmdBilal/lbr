...
app.get("/settings", checkAuthMiddleware, async(req,res)=>{
    try{
        ...
        res
        .status(200)
        .json({ success: true, data: result, message: "Setting get successfully." });
    }catch(err){
...
app.put("/upadte-setting", checkAuthMiddleware, async(req,res)=>{
    try {
        ...
        return res.status(200).json({success: true, message: "Setting updated successfully.", code: "Upadte_setting"});
      } catch (error) {
