...
app.post("/add_property",checkAminAuthMiddleware, async (req, res) => {
    try {
        ...
        const successMessage = req.body?.propertyFields.length === 1
...
// get api 
// list of property by questionnaire id
app.get("/listOfPropertiesByQuestionnaireId",checkAminAuthMiddleware, async(req,res) => {
    try{
        const questionnaireId = req.query.questionnaireId_id;
          let listofproperties = await ListOfPropertiesSchema.find({ questionnaireId: questionnaireId })
          .sort({ createdAt: -1})
