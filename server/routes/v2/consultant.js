...
app.post("/create-consultant", checkAminAuthMiddleware, async (req, res) => {
   try{
    ...
    let consultant_Data = {
        ...
        phone_no: phone_no,
        role: "consultant",
        image: "https://www.consultantsreview.com/newstransfer/upload/vn38rsz_business_consultant.jpg"
        // image: `${config.serverUrl}/public/images/${req.file?.filename}`,
    }
     await User.create(consultant_Data);
     res.status(200).json({
        success: true,
...
app.get("/get_consultant",checkAminAuthMiddleware, async (req, res)=>{
  try{
     ...
     .sort({ createdAt: -1 })
     .select("_id name phone_no email username ");
   res.status(200).json({
     data: result,
     success: true,
     message: "success",
